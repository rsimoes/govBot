...
def getMOLeg(partyDict):
    starterSoup = BeautifulSoup(urllib2.urlopen('http://www.senate.mo.gov').read(), 'lxml')
    url = starterSoup.find('a', {'rel': 'anylinkmenuSenators'}).get('href').replace('or-info', 'eroster')
    houseSoup = BeautifulSoup(urllib2.urlopen('http://www.house.mo.gov/member.aspx').read(), 'lxml')
    senateSoup = BeautifulSoup(urllib2.urlopen(url).read(), 'lxml')
...
if __name__ == "__main__":
    ...
    with open(writePath + 'MOLeg.csv', 'w') as csvFile:
