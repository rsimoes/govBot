...
##The MA Website lacks a standard district listing, this will catch most (but not all) district names
  ...
  table = houseSoup.find_all('tr', {'class': 'dataRow'}) + senateSoup.find_all('tr', {'class': 'dataRow'})
  ...
  for rep in table:
    ...
    rawBranch = rep.find('td', {'class': 'branchCol'})
    if rawBranch is None:
      branch = 'Senate'
    else:
    ...
    distList = BeautifulSoup(urllib2.urlopen(repInfo['Website']).read()).find('div', {'id': 'District'}).get_text().replace(' - ', ' -- ').replace(".","--").replace('consistng', '--').replace('consiting', '--').replace('consisting','--').replace('Consisting', '--').split('--')[0].strip().replace(',',' ').replace('   ', ' ').replace('  ', ' ').split(' ')
    ...
    if ordinalList[0].lower() in ordinalDict.keys():
      ...
      repInfo['District'] = 'MA State {0} District {1} {2}'.format(branch, distNum, townName)
