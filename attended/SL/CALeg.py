...
def getCALeg(partyDict):
    ...
    for item in houseTable:
        ...
        repInfo['District'] = 'CA State Assembly District {0}'.format(int(item.find('td', {'class': 'views-field views-field-field-member-district-value'}).get_text().strip()))
    ...
    for item in senateTable:
        ...
        repInfo['Name'] = re.sub(r'^(.*) \(.\)$', r'\1', rawName).replace(u'\u00A0', ' ').replace(u'\u0144', 'n').replace(u'\u00f1', 'n').replace(u'\u2018', "'").replace(u'\u2019', "'").replace(u'\u201A', "'").replace(u'\u201B', "'").replace(u'\u2039', "'").replace(u'\u203A', "'").replace(u'\u201C', '"').replace(u'\u201D', '"').replace(u'\u201E', '"').replace(u'\u201F', '"').replace(u'\u00AB', '"').replace(u'\u00BB', '"').replace(u'\u00e0', 'a').replace(u'\u00e1', 'a').replace(u'\u00e8', 'e').replace(u'\u00e9', 'e').replace(u'\u00ec', 'i').replace(u'\u00ed', 'i').replace(u'\u00f2', 'o').replace(u'\u00f3', 'o').replace(u'\u00f9', 'u').replace(u'\u00fa', 'u')
...
if __name__ == '__main__':
    ...
    with open(writePath + 'CALeg.csv', 'w') as csvFile:
