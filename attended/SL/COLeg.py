...
def getCOLeg(partyDict):
    response = urllib2.urlopen('http://www.leg.state.co.us/')
    session = re.sub(r'^.*?/clics/(.*?)/.*$', r'\1', response.url)
    for i in [1, 21, 41, 61, 81]:
        soup = BeautifulSoup(urllib2.urlopen('http://www.leg.state.co.us/clics/{1}/directory.nsf/Pink%20Book/l.%20All%20Legislators?OpenView&Start={0}&SimpleView=5'.format(i, session)).read())
        if i == 1:
            table = soup.find('table', {'cellpadding': '2'}).find_all('tr', {'valign': 'top'})
        else:
            table = table + soup.find('table', {'cellpadding': '2'}).find_all('tr', {'valign': 'top'})
    dictList = []
    for item in table:
        repInfo = {}
...
if __name__ == '__main__':
    ...
    with open(writePath + 'COLeg.csv', 'w') as csvFile:
